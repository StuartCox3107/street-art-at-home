...
The site was deployed to Heroku using the below steps:

- New app created on Heroku.com with an EU region
- Postgres database added to Heroku and all data migrated and loaded
- Relevant packages installed on the local environment (dj_database_url, psycopg2-binary, gunicorn)
- All modules in requirements.text
- Procfile created containing "web:gunicorn street-art-at-home.wsgi:application"
- Heroku config vars set as per the below
| Config Var | Detail | 
| --- | --- | 
| AWS_ACCESS_KEY | Generated by AWS | 
| AWS_SECRET_ACCESS_KEY | Generated by AWS | 
| DATBASE_URL | From Postgres | 
| SECRET_KEY | xxxxxx | 
| STRIPE_PUBLIC_KEY | From Strip | 
| STRIPE_SECRET_KEY | From Stripe | 
| STRIPE_WH_KEY | From Stripe | 
| USE_AWS | Set to true | 
Also, as I manually uploaded static and media files to AWS the below was also set
| DISABLE_COLLECTSTATIC | 1 | 
- Logged in to Heroku and pushed code
